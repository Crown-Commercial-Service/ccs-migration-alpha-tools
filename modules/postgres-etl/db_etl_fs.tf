resource "aws_efs_file_system" "db_etl" {
  ...
  tags = {
    "Name" = "${var.resource_name_prefixes.normal}:${upper(var.migrator_name)}"
    "TYPE" = "EFS"
  }
...
resource "aws_security_group" "db_etl_fs" {
  name        = "${var.resource_name_prefixes.normal}:${upper(var.migrator_name)}:EFS"
...
resource "aws_security_group" "db_etl_fs_clients" {
  name        = "${var.resource_name_prefixes.normal}:${upper(var.migrator_name)}:EFS:CLIENTS"
