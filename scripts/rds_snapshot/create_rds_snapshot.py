...
def create_rds_snapshot(rds_instance_name, region_name, desired_rds_instance_snapshot_status):
    ...
    try:
        ...
        create_snapshot = boto3_rds_client.create_db_snapshot(DBSnapshotIdentifier=rds_instance_snapshot_name, DBInstanceIdentifier=rds_instance_name)
        click.echo(f"CLI call made to create snapshot {rds_instance_snapshot_name}")
        rds_snapshot_instance_status = get_rds_snapshot_status(boto3_rds_client=boto3_rds_client, rds_instance_snapshot_name=rds_instance_snapshot_name)
        while rds_snapshot_instance_status != desired_rds_instance_snapshot_status:
            current_rds_snapshot_instance_status = get_rds_snapshot_status(boto3_rds_client=boto3_rds_client, rds_instance_snapshot_name=rds_instance_snapshot_name)
