resource "aws_security_group" "etl_extract_task" {
  name        = "${var.resource_name_prefixes.normal}:${upper(var.migrator_name)}:ECSTASK:EXTRACT" # ${var.resource_name_prefixes.normal}:PG_ETL:${upper(var.migrator_name)}:ECSTASK:EXTRACT
