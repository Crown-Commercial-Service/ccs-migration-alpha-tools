module "extract_task" {
  ...
  container_definitions = {
    pg_dump = {
      ...
      override_command = [
        "sh", "-c",
        "apk upgrade && rm -rf $MOUNT_POINT/$DUMP_FILENAME && pg_dump -d $DB_CONNECTION_URL -Fd -j ${var.extract_task_pgrestore_workers} --no-acl --no-owner -f $MOUNT_POINT/$DUMP_FILENAME && tar -cf $MOUNT_POINT/$DUMP_FILENAME.tar $MOUNT_POINT/$DUMP_FILENAME && aws s3 cp --quiet $MOUNT_POINT/$DUMP_FILENAME.tar s3://${var.s3_extract_bucket_name}-${var.environment_name}/$DUMP_FILENAME-$(date +%Y-%m-%d-%H-%M-%S).tar && echo \"$DUMP_FILENAME successfully uploaded to S3\""
      ]
      port = null
