module "extract_task" {
  ...
  container_definitions = {
    pg_dump = {
      ...
      override_command = [
        "sh", "-c",
        "apk upgrade && rm -rf $DUMP_FILENAME && pg_dump -d $DB_CONNECTION_URL --no-acl --no-owner | gzip > $DUMP_FILENAME.gz && aws s3 cp $DUMP_FILENAME.gz s3://${var.s3_bucket_name}-${var.environment_name}/etl-dump-$(date +%Y-%m-%d-%H-%M-%S).gz"
      ]
      port = null
