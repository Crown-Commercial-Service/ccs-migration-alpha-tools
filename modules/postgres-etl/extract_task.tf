module "extract_task" {
  ...
  container_definitions = {
    pg_dump = {
      ...
      override_command = [
        "sh", "-c",
        "apk upgrade && pg_dump -d $DB_CONNECTION_URL -Fd --file $DUMP_FILENAME --no-acl --no-owner && aws s3 cp $DUMP_FILENAME s3://${var.s3_bucket_name}-${var.environment_name}/etl-dump-$(date +%Y-%m-%d-%H-%M-%S)"
      ]
      port = null
